...
// Request timeout — 60 s to allow for full local LLM inference
...
export default function App() {
  ...
  const addMessage = useCallback((role, content) => {
    setMessages((prev) => [
      ...prev,
      { id: uid(), role, content, timestamp: now() },
    ]);
  }, []);
  ...
  const handleSend = useCallback(async (overrideText) => {
    ...
    try {
      ...
      if (!res.ok) {
        // Non-2xx HTTP response — show graceful error bubble
      ...
      addMessage('billy', data.response ?? FALLBACK_OFFLINE);

    } catch (err) {
      // Network error, timeout (AbortError), or JSON parse failure
      if (err.name === 'AbortError') {
        addMessage('error', 'Request timed out after 60 seconds. The model may still be loading — please try again.');
      } else {
